...
class triangle:
    def __init__(self, data=None, origin = None, dev = None, values = 'values', dataform = 'triangle'):
        ...
        self.origin = origin
        if origin == None:
            origin_in_col_bool, origin_in_index_bool = self.__set_origin(origin)   
        self.dev = dev     
        if dev == None:
            dev_in_col_bool = self.__set_dev(dev) 
    ...
    def dataAsTable(self, inplace=False):
        ...
        if self.dataform == 'triangle':
            for val in range(len(self.data.T.index)):
                df = pd.DataFrame(self.data.iloc[:,val].rename('values'))
    ...
    def dataAsTriangle(self, inplace=False):
        if self.dataform == 'tabular':
            tri = pd.pivot_table(self.data,values=self.values,index=[self.origin], columns=[self.dev]).sort_index()
