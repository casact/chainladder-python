...
class MackChainladder(Chainladder):
    ...
    def full_std_err_(self):
        ...
        val = xp.array([weight_dict.get(item.lower(), 2) for item in avg + [avg[-1]]])
    ...
    def _mack_recursion(self, est):
        ...
        nans = xp.concatenate((nans, xp.ones((*self.X_.shape[:3], 1)) * xp.nan), 3)
        nans = 1 - xp.nan_to_num(nans)
        properties = self._full_triangle_
        obj.ddims = np.concatenate(
            (properties.ddims[: len(self.X_.ddims)], np.array([properties.ddims[-1]]))
        )
        ...
        if est == "param_risk":
            ...
            obj.values = self._get_risk(nans, risk_arr, self.full_std_err_.values)
            obj._set_slicers()
    ...
    def _get_risk(self, nans, risk_arr, std_err):
        ...
        ldf = lxp.concatenate(
            (ldf, lxp.prod(self.X_.ldf_.values[..., -extend:], -1, keepdims=True)), -1
        )
    ...
    def _get_tot_param_risk(self, risk_arr):
        ...
        lxp = self.X_.ldf_.get_array_module()
        t1 = (
            xp.nan_to_num(self._full_triangle_.values)[..., : len(self.X_.ddims)]
            - xp.nan_to_num(self.X_.values)
            + xp.nan_to_num(
                (self.X_ - self.X_[self.X_.valuation < self.X_.valuation_date]).values
            )
        )
    ...
    def mack_std_err_(self):
        ...
        xp = obj.get_array_module()
        obj.values = xp.sqrt(
            self.parameter_risk_.values ** 2 + self.process_risk_.values ** 2
        )
        obj._set_slicers()
    ...
    def total_mack_std_err_(self):
        ...
        xp = obj.get_array_module()
        obj.values = xp.sqrt(
            self.total_process_risk_.values ** 2
            + self.total_parameter_risk_.values ** 2
        )
        obj.values = obj.values[..., -1:]
        return pd.DataFrame(
            obj.values[..., 0, 0], index=obj.kdims, columns=[item for item in obj.vdims]
        )
