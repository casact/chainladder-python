...
def weighted_regression_through_origin(x, y, w, axis):
    ...
    coef = np.expand_dims(coef, 3)
    sigma = np.expand_dims(np.sqrt(mse), 3)
    return np.concatenate((coef, sigma, std_err), 3)


def weighted_regression(y, axis, x=None, w=None):
    if w is None:
        w = np.ones(y.shape)
    if x is None:
        x = np.reshape(np.arange(y.shape[axis]), (1, 1, 1, y.shape[axis]))
        temp = np.array(x.shape)
        temp = np.where(temp == temp.max())[0][0]
        x = np.swapaxes(x, temp, axis) * w * (y*0 + 1)
    ...
    slope = np.expand_dims(slope, -1)
    intercept = np.expand_dims(intercept, -1)
