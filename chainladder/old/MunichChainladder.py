                     ...
                     'MCLvsMack':{'Title':'Munich Chainladder vs. Standard Chainladder (P/I) Ratio',
                      ...
                      'PI1':{'Title':'(P/I) Triangle using Basic Chainladder',
                                     ...
                                     'chart_type_dict':{'mtype':['plot', 'plot', 'plot'],
                                                       'x':[Triangle(self.MackPaid.full_triangle.iloc[:,:-1]).data_as_table()['dev_lag'], self.MackPaid.triangle.data_as_table()['dev_lag'],
                                                            lowess((Triangle(self.MackPaid.full_triangle.iloc[:,:-1]/self.MackIncurred.full_triangle.iloc[:,:-1]).data_as_table())['values'],
                                                                   Triangle(self.MackPaid.full_triangle.iloc[:,:-1]).data_as_table()['dev_lag'],frac=1 if len(np.unique(Triangle(self.MackPaid.full_triangle.iloc[:,:-1]).data_as_table()['dev_lag']))<=6 else 0.666).T[0]],
                                                       'yM':[(Triangle(self.MackPaid.full_triangle.iloc[:,:-1]/self.MackIncurred.full_triangle.iloc[:,:-1]).data_as_table())['values'],
                                                            ...
                                                            lowess((Triangle(self.MackPaid.full_triangle.iloc[:,:-1]/self.MackIncurred.full_triangle.iloc[:,:-1]).data_as_table())['values'],
                                                                   Triangle(self.MackPaid.full_triangle.iloc[:,:-1]).data_as_table()['dev_lag'],frac=1 if len(np.unique(Triangle(self.MackPaid.full_triangle.iloc[:,:-1]).data_as_table()['dev_lag']))<=6 else 0.666).T[1]],
                                                       'markerM':['o','o',''],
                                                       'linestyle':['','','-'],
                                                       ...
                                                       'y':[(Triangle(self.MackPaid.full_triangle.iloc[:,:-1]/self.MackIncurred.full_triangle.iloc[:,:-1]).data_as_table())['values'],
                                                            ...
                                                            pd.DataFrame(lowess((Triangle(self.MackPaid.full_triangle.iloc[:,:-1]/self.MackIncurred.full_triangle.iloc[:,:-1]).data_as_table())['values'],
                                                                   Triangle(self.MackPaid.full_triangle.iloc[:,:-1]).data_as_table()['dev_lag'],frac=1 if len(np.unique(Triangle(self.MackPaid.full_triangle.iloc[:,:-1]).data_as_table()['dev_lag']))<=6 else 0.666).T[1]).T],
                      ...
                      'PI2':{'Title':'(P/I) Triangle using Munich Chainladder',
                                     ...
                                     'YLabel':'Paid/Incurred Ratio',
                                     'chart_type_dict':{'mtype':['plot', 'plot','plot'],
                                                       'x':[Triangle(self.MCL_paid).data_as_table()['dev_lag'], self.MackPaid.triangle.data_as_table()['dev_lag'],
                                                            lowess((Triangle(self.MackPaid.full_triangle.iloc[:,:-1]/self.MackIncurred.full_triangle.iloc[:,:-1]).data_as_table())['values'],
                                                                   Triangle(self.MackPaid.full_triangle.iloc[:,:-1]).data_as_table()['dev_lag'],frac=1 if len(np.unique(Triangle(self.MackPaid.full_triangle.iloc[:,:-1]).data_as_table()['dev_lag']))<=6 else 0.666).T[0]],
                                                       'yM':[(Triangle(self.MCL_paid/self.MCL_incurred).data_as_table())['values'],
                                                            ...
                                                            lowess((Triangle(self.MCL_paid/self.MCL_incurred).data_as_table())['values'],
                                                                   Triangle(self.MCL_paid).data_as_table()['dev_lag'],frac=1 if len(np.unique(Triangle(self.MCL_paid).data_as_table()['dev_lag']))<=6 else 0.666).T[1]],
                                                       'markerM':['o','o',''],
                                                       'linestyle':['','','-'],
                                                       'colorM':['grey','blue', 'red'],
                                                       'type':['scatter', 'scatter', 'line'],
                                                       'y':[(Triangle(self.MCL_paid/self.MCL_incurred).data_as_table())['values'],
                                                            ...
                                                            pd.DataFrame(lowess((Triangle(self.MCL_paid/self.MCL_incurred).data_as_table())['values'],
                                                                   Triangle(self.MCL_paid).data_as_table()['dev_lag'],frac=1 if len(np.unique(Triangle(self.MCL_paid).data_as_table()['dev_lag']))<=6 else 0.666).T[1]).T],
                                                       'line_width':[None,None,2],
                                                       'line_cap':[None,None,'round'],
                                                       'line_join':[None,None,'round'],
                                                       'line_dash':[None,None,'solid'],
