...
class Chainladder():
    ...
    Parameters:
        ...
        delta : int
            A value representing an input into the weights of the WRTO class.
        tail : bool
            A value representing whether the user would like an exponential
            tail factor to be applied to the data.
    ...
    def set_LDF(self, LDF_average=None, colname_sep='-', inplace=False):
        ...
        Arguments:
            LDF_average: str or list
                Allows for the selection of 'volume', 'simple' 'regression',
                'geometric', and 'harmonic averages. Note that 'regression' is
                an OLS regression through the origin.  Specified as string will
                apply the same average to all development periods.  Specified
                as a list, each LDF can take on a different average. As a list,
                the length must be equal to the number of columns in the
                triangle - 1.
        ...
        if inplace:
            ...
            # Volume Weighted average and Straight Average, and regression
            # through origin
            _avg = {'regression': 0,
                    'volume': 1,
                    'simple': 2}
            val = np.array([[_avg.get(i.lower(), 2) for i in average]])
            val = np.repeat(val, self.triangle.n_origin_periods, axis=0)
            val = np.nan_to_num(val * (tri_array[:, 1:] * 0 + 1))
            # w = np.nan_to_num(w/tri_array[:,:-1]**(val))
            w = np.nan_to_num(w / (np.maximum(np.nan_to_num(tri_array[:, :-1]),
                                              1) *
                                   (tri_array[:, :-1] * 0 + 1)) ** val)
            LDF = np.sum(w * x * y, axis=0) / np.sum(w * x * x, axis=0)
            # Harmonic Mean
            w = np.array(self.weights.data_as_triangle().iloc[:, :-1])
            w_geom = np.logical_not(np.logical_not(w))
            w_harm = np.nan_to_num(w)
            x[x == 0] = np.inf
            ata = y / x * w_harm
            # Something is wrong with reciprocal function.  It seems there is
            # some mutation of the ata object going on.
            harmonic = ((np.sum(w_harm, axis=0) - 1) /
                        np.sum(np.reciprocal(ata, where=ata != 0), axis=0))
            ...
            geometric = (np.prod(geometric, axis=0) **
                         (1 / (np.sum(w_geom, axis=0) - 1)))
            # Array of product of Series is due to bypass an invalid numpy
            # warning - weird code...but it works
            LDF = (LDF * (average == 'volume') +
                   LDF * (average == 'simple') +
                   LDF * (average == 'regression') +
                   np.array(Series(average == 'geometric') *
                   Series(geometric)) +
                   np.array(Series(harmonic) * Series(average == 'harmonic')))
            LDF = np.append(LDF, [self.tail_factor])  # Need to modify for tail
            dev_periods = self.triangle.data_as_triangle().columns
            columns = [str(i1) + colname_sep + str(i2)
                       for i1, i2 in zip(dev_periods[:-1], dev_periods[1:])]
            columns.append(dev_periods[-1] + colname_sep + 'Ult')
            ...
            return new_instance.set_LDF(LDF_average=LDF_average,
                                        colname_sep=colname_sep, inplace=True)

    def select_average(self, LDF_average=None, num_periods=None,
                       inplace=False):
        """ Method to select number of years and the average used in LDF pick
        of the chainladder model.
        ...
        Arguments:
            ...
            num_periods: int or list
                Specify the number of origin periods to use in the LDF
                averaging.  Specified as a list, each LDF can take on a number
                of periods, and the list length must be equal to the number of
                columns in the triangle - 1.
            inplace: bool
                Set to true will mutate the Chainladder object, set to false
                will create a copy.
        ...
        """
        if inplace:
            temp = self.weights.data_as_triangle().values
            total_index = temp.shape[0]
            if num_periods is None:
                num_periods = np.repeat(total_index, temp. shape[1])
            if type(num_periods) == int:
                num_periods = np.repeat(min(num_periods, total_index),
                                        temp.shape[1])
            for i in range(temp.shape[1] - 1):
                try:
                    first_index = max(np.where(np.isnan(
                        temp[:, i]))[0][0] - num_periods[i] - 1, 0)
                except:
                    first_index = max(total_index - num_periods[i] - 1, 0)
                temp[:, i] = (np.append(np.repeat(0, first_index),
                                        np.repeat(1, total_index -
                                        first_index)) *
                              (temp[:, i] * 0 + 1))
            weights = DataFrame(temp, self.weights.data_as_triangle().index,
                                self.weights.data_as_triangle().index)
            self.weights = Triangle(weights)
            self.set_LDF(LDF_average, inplace=True)
            if self.method == 'DFM':
                self.DFM(self.triangle_pred, inplace=True)
            if self.method == 'born_ferg':
                self.born_ferg(self.exposure, self.apriori, self.triangle_pred,
                               inplace=True)
            if self.method == 'benktander':
                self.benktander(self.exposure, self.apriori, self.n_iters,
                                self.triangle_pred, inplace=True)
            if self.method == 'cape_cod':
                self.cape_cod(self.exposure, self.trend, self.decay,
                              self.triangle_pred, inplace=True)
            return self
        else:
            new_instance = copy.deepcopy(self)
            return new_instance.select_average(LDF_average=LDF_average,
    ...
    def DFM(self, triangle=None, inplace=False):
        ...
        Parameters:
            triangle: Triangle or None
                Set as None will apply the DFM to the triangle embedded in the
                Chainladder object. This can be overriden to apply the DFM
                method to a new triangle.
        ...
        if inplace:
            ...
            self.ultimates = Series((latest[:, 1] * np.array(self.CDF)[::-1]),
                                    index=triangle.data_as_triangle().index)
            return self
        else:
            new_instance = copy.deepcopy(self)
    ...
    def born_ferg(self, exposure, apriori, triangle = None, inplace=False):
        ...
        if inplace:
            ...
            CDF = np.array(self.CDF)[latest['dev_lag'].values.astype(int)-1]
            unreported_pct = 1 - 1 / CDF
            self.ultimates = Series(latest['values'].values + unreported_pct *
                                    my_apriori,
                                    index=triangle.data_as_triangle().index)
    ...
    def benktander(self, exposure, apriori, n_iters=1, triangle=None,
                   inplace=False):
        if inplace:
            if triangle is None:
                triangle = self.triangle
            ...
            for i in range(n_iters):
                temp_ult = self.born_ferg(exposure=temp_ult, apriori=1,
                                          triangle=triangle).ultimates
            ...
            new_instance = copy.deepcopy(self)
            return new_instance.benktander(exposure=exposure, apriori=apriori,
                                           n_iters=n_iters, triangle=triangle,
                                           inplace=True)

    def cape_cod(self, exposure, trend=0, decay=1, triangle=None,
                 inplace=False):
        if inplace:
            if triangle is None:
                triangle = self.triangle
            latest = triangle.get_latest_diagonal()[exposure > 0]
            CDF = np.array(self.CDF)[latest['dev_lag'].values.astype(int)-1]
            reported_exposure = np.array(exposure[exposure > 0]) / CDF
            trend_array = np.array([(1+trend)**(np.sum(exposure > 0) - (i+1))
                                    for i in range(np.sum(exposure > 0))])
            decay_matrix = np.array([[decay**abs(i-j)
                                      for i in range(np.sum(exposure > 0))]
                                     for j in range(np.sum(exposure > 0))])
            weighted_exposure = reported_exposure * decay_matrix
            trended_ultimate = np.repeat(np.array([(latest['values'].values *
                                                    trend_array) /
                                                   reported_exposure]),
                                         np.sum(exposure > 0), axis=0)
            apriori = (np.sum(weighted_exposure * trended_ultimate, axis=1) /
                       np.sum(weighted_exposure, axis=1))
            detrended_ultimate = apriori/trend_array
            IBNR = (detrended_ultimate * (1-1/CDF) *
                    np.array(exposure[exposure > 0]))
            self.ultimates = Series(latest['values'].values + IBNR,
                                    index=exposure[exposure > 0].index)
            ...
            new_instance = copy.deepcopy(self)
            return new_instance.cape_cod(exposure=exposure, trend=trend,
                                         decay=decay, triangle=triangle,
                                         inplace=True)
    ...
    def exclude_link_ratios(self, otype, inplace=False):
        if inplace:
            if type(otype) is list:
                ...
                for item in otype:
                    self.weights.data_as_triangle().iloc[item] = 0
            else:
                ata = self.age_to_age().iloc[:-3]
                ata.index = self.weights.data_as_triangle().index
                ata.columns = self.weights.data_as_triangle().columns
                if otype == 'min':
