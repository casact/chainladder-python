...
def test_mack_tail_ldf(data, averages, est_sigma, atol):
    ...
    p = xp.concatenate((p.values[0, 0, :, :][:,:-2], xp.prod(p.values[0, 0, :, :][:, -2:],-1, keepdims=True)), -1)
...
def test_tail_doesnt_mutate_ldf_(data, averages, est_sigma):
    ...
    xp.testing.assert_array_equal(p_no_tail, p.values[..., :len(cl.load_sample(data).ddims)-1])
...
def test_fit_period():
    ...
    assert round(cl.TailCurve(fit_period=(tri.ddims[-7], None), extrap_periods=10).fit(dev).cdf_['paid'].set_backend('numpy', inplace=True).values[0,0,0,-2],3) == 1.044
