...
class Chainladder():
    ...
    def __init__(self, tri, weights=1, delta=1, tail=False):
        ...
        dev_periods = tri.data.index.levels[1]
        self.delta = [delta for _ in dev_periods]
    ...
    def predict(self):
        ...
        Returns:
            pandas.DataFrame representing the raw triangle data as well as
            future lags populated with the expectation from the chainladder
            fit.
        """
        ldf = [item.coefficient for item in self.models]
        square = self.triangle.data_as_triangle()
    ...
    def fit(self):
        ...
        """
        models = []
        dev_periods = self.triangle.data.index.levels[1]
        tri = self.triangle.data_as_triangle()
        weights = self.weights.data_as_triangle()
        for i, _ in enumerate(dev_periods[:-1]):
            data = tri.iloc[:, i:i + 2].dropna()
            w = weights.iloc[:, i].dropna().iloc[:-1]
            X = data.iloc[:, 0]
            y = data.iloc[:, 1]
    ...
    def get_LDF(self, colname_sep='-'):
        ...
        """
        triangle = self.triangle.data_as_triangle()
        LDF = Series([ldf.coefficient for ldf in self.models],
                     index=[str(item) + colname_sep +
                     str(triangle.columns.values[num + 1]) for num,
                     item in enumerate(triangle.columns.values[:-1])])
        if len(LDF) >= len(triangle.columns)-1:
            return Series(LDF[:len(triangle.columns)-1])
        else:
            return LDF
    ...
    def get_tail_factor(self, colname_sep='-'):
        ...
        """
        triangle = self.triangle.data_as_triangle()
        LDF = np.array(self.get_LDF()[:len(triangle.columns)-1])
        if not self.tail:
            tail_factor = 1
        elif len(LDF[LDF > 1]) < 2:
            warn("Not enough factors larger than 1.0 to fit an exponential "
                 "regression.")
            tail_factor = 1
            ...
            tail_factor = np.product(np.exp(tail_model.params[0] +
                                     np.array([i+2 for i in range(n, n+100)]) *
                                     tail_model.params[1]).astype(float) + 1)
            if tail_factor > 2:
                warn("The estimate tail factor was bigger than 2 and has been "
                     "reset to 1.")
                tail_factor = 1
            if tail_model.f_pvalue > 0.05:
                warn("The p-value of the exponential tail fit is insignificant"
                     " and tail has been set to 1.")
                tail_factor = 1
        else:
            warn("LDF[-2] * LDF[-1] is not greater than 1.0001 and tail has "
                 "been set to 1.")
            tail_factor = 1
        return Series(tail_factor,
                      index=[str(triangle.columns[-1]) + colname_sep + 'Ult'])
