...
def test_drops():
    ...
    assert np.all(
        np.around(
            cl
            .BarnettZehnwirth(formula="C(development)", drop_valuation="1979")
            .fit_transform(abc)
            .ldf_.values,
            3,
        )
        == np.around(
            cl
            .BarnettZehnwirth(
                formula="C(development)",
                drop=[("1977", 36), ("1978", 24), ("1979", 12)],
            )
            .fit_transform(abc)
            .ldf_.values,
            3,
...
def test_bz_2008():
    ...
    model = cl.BarnettZehnwirth(
        drop=("1982", 72), alpha=origin_buckets, gamma=dev_buckets, iota=val_buckets
    ).fit(abc_adj)
    assert np.all(
